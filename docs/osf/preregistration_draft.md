...
Primary databases per the original proposal: PubMed/MEDLINE, Embase, IEEE Xplore, ACM Digital Library (4 databases). **Deviation (2026-06-22): Embase dropped — no institutional access.** UNO (the reviewer's home institution) does not subscribe to Embase; UNMC subscribes, but the reviewer has no enrollment, affiliation, or other authorized path to UNMC's license, and no librarian-mediated search was available within the Phase 1 timeline. Search is therefore being executed across **3 databases: PubMed/MEDLINE, IEEE Xplore, ACM Digital Library**. This is logged as a documented protocol deviation (`memos/decision_log.md`, 2026-06-22) and will be reported as a limitation in the manuscript. CINAHL, included during the exploratory cross-domain phase, is shelved as a primary source under the EM-only scope (`memos/decision_log.md`, 2026-06-10) and may be used as a Phase 3 sensitivity check if time allows.

PubMed search string (Concept A: XAI/interpretability terms, AND Concept B: clinical/CDS context terms, AND Concept C: ED/triage/ESI/acuity/disposition terms) is documented in `docs/protocol/search_string_pubmed_v1.md` (v2 FINAL, 2026-06-10) and formally executed 2026-06-10 (497 records). IEEE Xplore was formally executed 2026-06-18 (161 records, after a wildcard-limit fix — see `docs/protocol/search_string_ieee_v1.md`). **ACM Digital Library was executed 2026-06-22 (1 record) via a documented methodology substitution**: ACM's native Advanced Search syntax could not be confirmed working (0 results even for single-term control queries), so the search was instead run against the OpenAlex API restricted to ACM as publisher — a different system indexing the same underlying catalog, with ~98% abstract coverage for the corpus searched. See `docs/protocol/search_string_acm_v1.md` and `memos/decision_log.md` (2026-06-22) for the full rationale and diagnostic trail. Embase translation is retained in `docs/protocol/search_string_embase_v1.md` for the record but will not be executed per the access deviation noted above.
...
| Draft v2.2 | 2026-06-22 | **Database scope reduced from 4 to 3: Embase dropped due to no institutional access** (UNO does not subscribe; UNMC does, but the reviewer has no authorized access path). PubMed (497 records) and IEEE Xplore (161 records, after a wildcard-limit fix) formally executed. **ACM Digital Library executed via a documented methodology substitution** (OpenAlex API restricted to ACM as publisher, after ACM's native Advanced Search syntax failed control-term testing) — 1 record. Both deviations logged for the manuscript limitations section. See `memos/decision_log.md`, 2026-06-22. |
